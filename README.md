...
- 网格周课表：横轴是星期 + 日期列（只显示有课的星期），纵轴是每小时一个刻度的固定时间轴（整点实线 + 半点虚线），时间范围自动对齐当天最早/最晚课程。
- 每节课是落在格子里的彩色竖长方形块：高度 = 真实时长，位置 = 真实起止时间，课间空隙一目了然；同一时段冲突的课自动并排。
- 课程块颜色按课程名自动分配（同一门课始终同色）；块内显示课程名、起止时间、地点。
- 「今天」所在列有绿色高亮和标签；当天没课时头部显示「没课 🎉」。
- 顶部下拉可跳转任意一周；直达链接支持 `?week=3`；手机上星期列较多时可左右滑动。
